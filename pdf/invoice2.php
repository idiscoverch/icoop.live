...
if (isset($_GET['inv2_date'])) { $invoice_date=$_GET['inv2_date']; }
...
if (isset($_GET['zahlungskonditionen'])) { $zahlungskonditionen=$_GET['zahlungskonditionen']; }
...
if($ord_schedule_id){
	$sql_header = "select v_order_schedule.order_nr,
		...
		v_order_schedule.arrival_month_year,
	...
	$arrival_month = $row_header['arrival_month_year'];
	...
	$country_de = $row_header['country_de'];
	$month_eta = $row_header['month_eta'];
	$proposal_currency_id = $row_header['proposal_currency_id'];
	
	
	$sql_detail1 = "select id, container_nr, vgm_weight, vgm_diff, vgm_delivery from (       
	select row_number() over () as id, container_nr,
	to_char(l.vgm_weight/1000,'999G999D999') vgm_weight,
	to_char(l.vgm_diff /1000,'999G990D999') vgm_diff,
	to_char(l.vgm_delivery /1000,'999G999D999') vgm_delivery
	...
	to_char(l.vgm_delivery /1000,'999G999D999') vgm_delivery
	 FROM ord_con_booking b, ord_con_list l, ord_con_loading_item i
	where b.ord_schedule_id=$ord_schedule_id and b.booking_segment=1 and l.con_booking_id=b.id_con_booking and i.con_list_id=l.id_con_list ) q2 )
	";
	
	$sql_detail2 = "select id, container_nr, vgm_weight, vgm_diff, vgm_delivery from (      
    select row_number() over () as id, container_nr,
    to_char(l.vgm_weight /1000,'999G999D999') vgm_weight,
    to_char(l.vgm_diff /1000,'999G990D999') vgm_diff,
    to_char(l.vgm_delivery /1000,'999G999D999') vgm_delivery
	...
	$sql_total = "select h.total_vgm_weight,
	to_char((h.total_vgm_weight*c.proposal_price),'999G999D999') total_invoice,
	h.vgm_deliver_total, 
	h.vgm_diff_total, 
	c.ship_sales_value_tone AS proposal_price, 
	to_char((h.vgm_diff_total*c.ship_sales_value_tone),'999G999D99') total_diff
	...
	$proposal_price = $row_total['proposal_price'];
	$total_invoice = $row_total['total_invoice'];
	$vgm_deliver_total = $row_total['vgm_deliver_total'];
	$vgm_diff_total = trim($row_total['vgm_diff_total']);
...
if($total_diff>0){
	...
	$PDF->Cell(3,1,'Gutschrift','C');
}

$PDF->Cell(48);
$PDF->SetTextColor(0,0,0);
$PDF->Cell(3,1,trim($invoice_number),'C');
$PDF->Ln(15);
$PDF->SetFont('HelveticaNeueLight','',10);
$PDF->SetTextColor(103,106,108);
...
if(($due_date)&&($total_diff>0)){
	$PDF->Ln(7);
	$PDF->SetTextColor(103,106,108);
	$PDF->Cell(3,1,'Zahlungsziel','C');
	$PDF->Cell(48);
	$PDF->SetTextColor(0,0,0);  
	$PDF->Cell(3,1,$due_date,'C');
}

$PDF->Ln(7);
$PDF->SetFont('HelveticaNeueLight','',10);
$PDF->SetTextColor(103,106,108);
$PDF->Cell(3,1,'Käufer','C');
$PDF->Cell(48);
$PDF->SetTextColor(0,0,0);
...
$PDF->Cell(48);
$PDF->SetTextColor(0,0,0);
$PDF->Cell(3,1,trim($arrival_month),'C');    
$PDF->Ln(7);
$PDF->SetFont('HelveticaNeueLight','',10);
$PDF->SetTextColor(103,106,108);
...
$PDF->SetFont('HelveticaNeueLight','',10);
$PDF->SetTextColor(103,106,108);
$PDF->Cell(3,1,'Qualität','C');
$PDF->Cell(48);
$PDF->SetTextColor(0,0,0);
$PDF->Cell(3,1,utf8_decode(trim($var_quality)),'C');
$PDF->Ln(7);
$PDF->SetFont('HelveticaNeueLight','',10);
$PDF->SetTextColor(103,106,108);
...
// $PDF->Ln(6);
// $PDF->Cell(51);
// $PDF->Cell(3,1,'Allgemeine Verkaufsbedingungen siehe Rückseite','C');
$PDF->Ln(7);

// if($total_diff>0){
	// if($zahlungskonditionen){
		// $PDF->SetFont('HelveticaNeueLight','',10);
		// $PDF->SetTextColor(103,106,108);
		...
		// $x=1;
		// foreach (explode('@@', $zahlungskonditionen) as $val) {
			// if($x==1){ $PDF->Cell(48); }
			// else { $PDF->Cell(51); }
			// $PDF->Cell(3,1,$val,'C');
			// $PDF->Ln(5);
...
$PDF->SetFont('HelveticaNeueLight','',10);
$PDF->SetTextColor(103,106,108);
$PDF->Cell(3,1,'Menge / Differenz','C');
$PDF->Cell(48);
$PDF->SetTextColor(0,0,0);  
$PDF->Cell(3,1,$vgm_diff_total.' MT Rohware - '.trim($currency).' '.$proposal_price.' / MT - Differenz '.trim($currency).' '.$total_diff,'C');

$PDF->Ln(12);
$PDF->SetFont('HelveticaNeueLight','',10);
$PDF->SetTextColor(103,106,108);
$PDF->Cell(3,1,'Gewichtsdifferenz','C');
$PDF->Cell(48);
$PDF->SetTextColor(0,0,0);
...
while ($row_detail1 = pg_fetch_assoc($rs_detail1)){
	array_push($data1, $row_detail1['container_nr']."#".trim($row_detail1['vgm_weight']).'#'.trim($row_detail1['vgm_delivery']).'#'.trim($row_detail1['vgm_diff']));
}
$nb1=count($data1);

$data2=array();
while ($row_detail2 = pg_fetch_assoc($rs_detail2)){
	array_push($data2, $row_detail2['container_nr']."#".trim($row_detail2['vgm_weight']).'#'.trim($row_detail2['vgm_delivery']).'#'.trim($row_detail2['vgm_diff']));
}
$nb2=count($data2);
...
// if(($data1)&&($data2)){
	...
	$tabhcont[7] = 15;
	$tabhcont[8] = 12;
	$tabhcont[9] = "[LB]Cont. Nr.";
	$tabhcont[10] = "[LB]Versand";
	$tabhcont[11] = "[RB]Ankunft";
	$tabhcont[12] = "[RB]Diff";
	$tabhcont[13] = "[RB] ";
	$tabhcont[14] = "[LB]Cont. Nr.";
	$tabhcont[15] = "[LB]Versand";
	$tabhcont[16] = "[RB]Ankunft"; 
	$tabhcont[17] = "[RB]Diff";
// } else {
	// $tabhcont[0] = 27;
	// $tabhcont[1] = 15;
	...
	// $tabhcont[3] = 12;
	// $tabhcont[4] = 70;
	// $tabhcont[4] = "[LB]Cont. Nr.";
	// $tabhcont[5] = "[LB]Versand";
	// $tabhcont[6] = "[RB]Ankunft";
	// $tabhcont[7] = "[RB]Diff";
	// $tabhcont[9] = "[RB] ";
// }
...
{ 
	...
	if(($data1)&&($data2)){
		...
		$tabvalues[]="[L]".$vgm_weight2;
		$tabvalues[]="[R]".$vgm_delivery2;
		$tabvalues[]="[R]".$vgm_diff2;
		$tabvalues[]="[R] ";
		$tabvalues[]="[L]".$container_nr1;
		$tabvalues[]="[L]".$vgm_weight1;
		$tabvalues[]="[R]".$vgm_delivery1;
		$tabvalues[]="[R]".$vgm_diff1;
...
if($total_diff>0){
	...
	$text='Gutschriftsbetrag';
}

$PDF->SetTextColor(0,0,0);
$PDF->Cell(3,1,'Differenz ('.$vgm_diff_total.' MT) - '.$text,'C');
// $PDF->Cell(166);
// $PDF->Cell(3,1,$currency.' '.$total_invoice,'C');
$PDF->Cell(0,1,trim($currency).' '.$total_diff,0,0,'R');
...
$tabhcont2[4] = 30;
$tabhcont2[5] = 25;
$tabhcont2[6] = "[LB]Container-Nr.";
$tabhcont2[7] = "[LB]Tara";
$tabhcont2[8] = "[RB]LadeGewicht";
$tabhcont2[9] = "[RB]Brutto Gewicht";
$tabhcont2[10] = "[RB]Ankunft Gewicht";
$tabhcont2[11] = "[RB]Diff";
...
while ($row_container_onward = pg_fetch_assoc($rs_container_onward)) {
	$tabvalues2[]="[L]".$row_container_onward['container_nr'];
	$tabvalues2[]="[L]".$row_container_onward['tare'];
	$tabvalues2[]="[R]".$row_container_onward['vgm_weight'];
	$tabvalues2[]="[R]".$row_container_onward['gross_weight_arrival'];
	$tabvalues2[]="[R]".$row_container_onward['vgm_delivery'];
	$tabvalues2[]="[R]".$row_container_onward['vgm_diff'];
}
