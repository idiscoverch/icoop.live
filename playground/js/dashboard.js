...
function rightPanel(cult,pays,produit,supplier,farmer,surface) {
	...
	var resurl2='listeslies.php?elemid=timeline';
	var xhr2 = getXhr();
	...
	xhr2.onreadystatechange = function(){
		if(xhr2.readyState == 4 ){
			...
			document.getElementById('acc').innerHTML = leselect2;
...
L.easyButton('fa-reply', function () {
    ...
    history.pushState(stateObj, "page 2", "dashboard.php");
...
function showVessel(ship_name) {
	
	clear_map_layer();
	
	var vessel_route = [];
	var resurl='listeslies.php?elemid=show_vessel_line&ship_name='+ship_name;
	var xhr = getXhr();
	...
	xhr.onreadystatechange = function(){
		...
		if(xhr.readyState == 4 ){
			...
			while (val[i]) { 
				var val1 = val[i].split('#'); 
				vessel_route.push([val1[0],val1[1]]);  
				i +=1;
			}  
		 
			for (var e = 0, latlngs = [], len = vessel_route.length; e < len; e++) {
				latlngs.push(new L.LatLng(vessel_route[e][0], vessel_route[e][1]));
			}

			path = L.polyline(latlngs, {snakingSpeed: 100}).addTo(route);

			map.fitBounds([latlngs[0],latlngs[len - 1]],{maxZoom : 3});
...
function showPlantations() {
	...
	var resurl='listeslies.php?elemid=show_plantations'; 
	var xhr = getXhr();
	...
	xhr.onreadystatechange = function(){ 
		if(xhr.readyState == 4 ){
			...
			var poly = JSON.parse(leselect); 
			plantation_couche.addData(poly);
...
function showBuyers(){
	...
	var resurl='listeslies.php?elemid=show_buyers';
	var xhr = getXhr();
	...
	xhr.onreadystatechange = function(){
		if(xhr.readyState == 4 ){
			...
			while (val[i] != 'end') {      
				...
				var pulsingIcon = L.icon.pulse({iconSize:[10,10],color:'yellow'});
				mark1 =  L.marker([val2[0], val2[1]],{icon: pulsingIcon}).addTo(city_couche);
				mark = L.marker([val2[0], val2[1]],{icon: new SweetIcon1({iconUrl: 'images/icon.png',labelText:val2[2]})   ,riseOnHover:false}).addTo(labels);
				
                map.addLayer(city_couche); 
				map.fitBounds(city_couche.getBounds());
