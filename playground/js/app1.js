...
var protected_area = L.geoJson(null, {
	style: function (feature) {
		return {
			color: 'red',
			fillColor: 'green',
...
function onEachFeature_area(feature, layer) {
	var popupContent = layer.feature.properties.name_area;
...
function onClick_village(e) {
		...
		if ((json_village[i].properties.y == latitude) && (json_village[i].properties.x == longitude)){
			...
			if(json_village[i].properties.e0_01photo != null && json_village[i].properties.e0_01photo != ''){
   			...
   			content += '<div style="background-color: #20b2aa;color: #fff;padding:10px 20px">'+blanc
				...
				+'<h3 style="color:#ffa500" class="pull-left">&nbsp;'+lg_farmers_list+'</h3><span id="toggle_plant">'+html_show_plant+'</span>'+blanc
	...
	$("#ionrange_equipement").on("change", function () {
		...
		create_circle(centre_y,centre_x,value);
	});

	create_circle(centre_y,centre_x,0);
	map.setZoom(12);
}
...
function checked_layer(type,valeur){
	...
	if(type == 'aire_protegee'){
		if(valeur.checked == true){
			map.addLayer(protected_area);

		} else {
			map.removeLayer(protected_area);
		}
	}
}
...
// alert(story_couche.getBounds());
			
			
			
			 // map.fitBounds([[coord[0],coord[1]],[coord[0]+0.00000000000000000001,coord[1]+0.0000000000000000000001]]);
// alert(7);
			if(step_v != 1){
...
$.getJSON("data/protected_area.geojson", function (data) {
	protected_area.addData(data);
});
...
// $.getJSON("data/label_time.geojson", function (data) {
	// fuseau.addData(data);
// });
