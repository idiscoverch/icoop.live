...
function longin_connexion(){
	...
	if (username == '' || password == ''  ){
		...
		var resurl='playground_listeslies.php?elemid=connexion&username='+username+'&password='+password;
        var xhr = getXhr();
...
var map = new L.Map('map', {layers: [cartodb_light], maxZoom: 22});
...
function ticker(code,buyer) {
	var resurl='playground_listeslies.php?elemid=ticker&code='+code+'&buyer='+buyer;
	var xhr = getXhr();
...
$(document).ready(function() {
	var mydata  = [];
	var resurl='playground_listeslies.php?elemid=ticker&code=&culture=';
	var xhr = getXhr();
...
var SweetIcon = L.Icon.Label.extend({
	options: {
		iconUrl: 'playground/images/icon.png',
		shadowUrl: null,
...
var village_cluster = L.markerClusterGroup({spiderfyOnMaxZoom: true, showCoverageOnHover: true, zoomToBoundsOnClick: true});  
// var village_cluster = L.geoJson(null, { });
...
var capitale = L.geoJson(null, {
	pointToLayer: function (feature, latlng) {
		...
		mark = L.marker(latlng,{icon: new SweetIcon({iconUrl: 'playground/images/icon.png',labelText: feature.properties.name_country})	,riseOnHover:false}).addTo(labels);
	...
	pointToLayer: function (feature, latlng) {
		if (feature.properties.type == 'hopital'){
			return L.marker(latlng, {
				icon: L.icon({
					iconUrl: "img/playground/hopital.png",
					iconSize: [15, 15],
					iconAnchor: [12, 28],
					popupAnchor: [0, -25]
				}),
			...
			return L.marker(latlng, {
				icon: L.icon({
					iconUrl: "img/playground/pharmacie.png",
					iconSize: [15, 15],
					iconAnchor: [12, 28],
					popupAnchor: [0, -25]
				}),
...
var ecole = L.geoJson(null, {
  pointToLayer: function (feature, latlng) {
    return L.marker(latlng, {
      icon: L.icon({
        iconUrl: "img/playground/ecole.png",
        iconSize: [35, 35],
        iconAnchor: [12, 28],
        popupAnchor: [0, -25]
      }),
...
var pays_couche = L.geoJson('', { style:style, onEachFeature: function (feature, layer) {
	...
	if (culture == 0) {
		...
		mark = L.marker([layer.feature.properties.capitale_y, layer.feature.properties.capitale_x],{
			icon: new SweetIcon({iconUrl: 'playground/images/icon.png',labelText:lang_pays})   ,riseOnHover:false
...
var exporter_couche = L.geoJson(null, {
    pointToLayer: function (feature, latlng) {
		...
		mark = L.marker(latlng,{icon: new SweetIcon({iconUrl: 'playground/images/icon.png',labelText: feature.properties.initials})	,riseOnHover:false}).addTo(labels);
...
var city_couche = L.geoJson(null, {
    pointToLayer: function (feature, latlng) {
		var pulsingIcon = L.icon.pulse({iconSize:[8,8],color:'blue'});
		mark = L.marker(latlng,{icon: new SweetIcon({iconUrl: 'playground/images/icon.png',labelText: feature.properties.name_city})	,riseOnHover:false}).addTo(labels);
...
var buyer_couche = L.geoJson(null, {
    pointToLayer: function (feature, latlng) {
		var pulsingIcon = L.icon.pulse({iconSize:[8,8],color:'blue'});
		mark = L.marker(latlng,{icon: new SweetIcon({iconUrl: 'playground/images/icon.png',labelText: feature.properties.name_buyer})	,riseOnHover:false}).addTo(labels);
...
var village_couche = L.geoJson(null, {
	pointToLayer: function (feature, latlng) {
		var pulsingIcon = L.icon.pulse({iconSize:[8,8],color:'blue'});
		mark = L.marker(latlng,{icon: new SweetIcon({iconUrl: 'playground/images/icon.png',labelText: feature.properties.name_town})	,riseOnHover:false}).addTo(labels);
...
var story_couche = L.geoJson(null, {
    pointToLayer: function (feature, latlng) {
		var pulsingIcon = L.icon.pulse({iconSize:[8,8],color:'blue'});
		mark = L.marker(latlng,{icon: new SweetIcon({iconUrl: 'playground/images/icon.png',labelText: feature.properties.no_step})	,riseOnHover:false}).addTo(labels);
	...
	for (j in json_exporter) {
        if ((json_exporter[j].properties.coord_y == longitude) && (json_exporter[j].properties.coord_x == latitude)){
			...
			if(json_exporter[j].properties.e0_02logo != null && json_exporter[j].properties.e0_02logo != ''){
				content += '<img src="img/playground/'+json_exporter[j].properties.e0_02logo+'" height="60" style="box-shadow:0 1px 6px 0 rgba(0,0,0,.3);position:absolute;right:20px;margin-top:180px;float:right"/>';
			}

			if(json_exporter[j].properties.e0_01photo != null && json_exporter[j].properties.e0_01photo != ''){
				content += '<img height="200px" src="img/playground/'+json_exporter[j].properties.e0_01photo+'" style="width:100%;"/></div>';
			}
			...
			content +='<div style="background-color: #4169e1;color: #fff;padding:10px 20px">'+blanc
					...
					+'<img src="img/playground/'+json_exporter[j].properties.code_country+'.png" height="12" style=""/>&nbsp;<strong>'+json_exporter[j].properties.e0_05+'</strong>'+blanc
...
function imgManagement(val,id) {
	var resurl='playground_listeslies.php?elemid=imgManagement&val='+val+'&id='+id;
	var xhr = getXhr();
...
function pays(code_pays,culture) { 
	...
	if(id_buyer!=''){ 
		...
		if(id_exporter !== 0){
			for (j in json_exporter) {
				if(json_exporter[j].properties.id_contact == id_exporter){
					if(idview == 1){ var exporter=json_exporter[j].properties.name_exporter; } else { var exporter=json_exporter[j].properties.initials; }
	...
	var resurl='playground_listeslies.php?elemid=stats&code='+code_pays;
	var xhr = getXhr();
	...
	for (i in json_stories) {
		if(( code_pays == json_stories[i].properties.code) && ( culture == json_stories[i].properties.id_culture)){
			...
			if( json_stories[i].properties.media_type == 1 ){
				...
				link = '<img alt="image" style="width:100%;" class="img-responsive" src="img/playground/story/'+json_stories[i].properties.media_link.replace(/\s+/g, '')+'">';
...
var pointIcon = new LeafIcon({iconUrl: 'img/playground/icon_point.png'}) ;
		...
		if ((json_village[i].properties.x == latitude) && (json_village[i].properties.y == longitude)){
			...
			plantation_couche0.eachLayer(function (layer) {
				if(layer.feature.properties.code_farmer == json_village[i].properties.code_farmer){
					...
					if(layer.feature.properties.geojson!="") {
						k+=1;
					}
				}
			});
			...
			if(json_village[i].properties.name_farmer){
				var f_name = json_village[i].properties.name_farmer.substr(0, 5)+'...';
			} else { var f_name = ""; }
                ...
                +'<div class="panel panel-default">'+blanc
                    +'<div class="panel-heading">'+blanc
                        +'<table style="width:100%" class="panel-title"><tr>'+blanc
						  +'<td style="width:25%">'+f_name+'</td>'+blanc
			...
			if(json_village[i].properties.e0_01photo != null && json_village[i].properties.e0_01photo != ''){
				content += '<img height="200px" src="img/playground/'+json_village[i].properties.e0_01photo+'" style="width:100%;"/></div>';
			}

   			content += '<div style="background-color: #20b2aa;color: #fff;padding:10px 20px">'+blanc
				+'<h3>'+towns+' </h3>'+blanc
				   +'<img src="img/playground/'+var_pays+'.png" height="12" style=""/>&nbsp;<strong>'+lg_farmervillage+'</strong>'+blanc
				...
				+'<br><div class="panel panel-default" style="margin-bottom: 10px;padding: 5px 10px;width:100%;opacity:0.8">'+blanc
					...
					+'&nbsp;&nbsp;<input id="sante_checkbox" onclick="checked_layer(\'sante\',this);" class="leaflet-control-layers-selector" type="checkbox"> &nbsp;<label for="sante_checkbox"><img src="img/playground/hopital_small.png">&nbsp;'+lg_helf+'</label>&nbsp;&nbsp;&nbsp;&nbsp; '+blanc
					+'<input id="ecole_checkbox" onclick="checked_layer(\'ecole\',this);" class="leaflet-control-layers-selector" type="checkbox">&nbsp;<label for="ecole_checkbox"><img src="img/playground/ecole_small.png">&nbsp;'+lg_schools+'</label>&nbsp;&nbsp;'+blanc
					// +'<br>&nbsp;&nbsp;<input id="aire_protegee_checkbox" onclick="checked_layer(\'aire_protegee\',this);" class="leaflet-control-layers-selector" type="checkbox"> &nbsp;<label for="aire_protegee_checkbox"><img src="img/playground/protected_area_small.png">&nbsp;'+lg_protected_areas+'</label>&nbsp;&nbsp;'+blanc
...
function onClick_buyer(e) {
	...
	for (i in json_buyer) {
		if ((json_buyer[i].properties.coord_y == longitude) && (json_buyer[i].properties.coord_x == latitude)){
			...
			if(json_buyer[i].properties.e0_02logo != null && json_buyer[i].properties.e0_02logo != ''){
				content += '<img src="img/playground/'+json_buyer[i].properties.e0_02logo+'" height="60" style="box-shadow:0 1px 6px 0 rgba(0,0,0,.3);position:absolute;right:20px;margin-top:180px;float:right"/>';
			}

			if(json_buyer[i].properties.e0_01photo != null && json_buyer[i].properties.e0_01photo != ''){
				content += '<img height="200px" src="img/playground/'+json_buyer[i].properties.e0_01photo+'" style="width:100%;"/></div>';
			}

			content +='<div style="background-color: #dae456;color: #fff;padding:10px 20px">'+blanc
				+'<h3>'+buyer+' </h3>'+blanc
				+'<img src="img/playground/'+json_buyer[i].properties.code_country+'.png" height="12" style=""/>&nbsp;<strong>'+json_buyer[i].properties.e0_05+'</strong>'+blanc
				+'<br><div style="display:inline-block"><div class="pull-left">'+lg_local_time+' : &nbsp;&nbsp; </div><div id="time-cont-1" class="pull-left"></div> &nbsp;&nbsp; '+gmt+'</div>'+blanc
				+'</div>'+blanc
			+'<div style="padding:10px 20px; border-bottom:1px solid #e7eaec;width:100%">';
			...
			for (j in json_village_group2) {
				...
				var pulsingIcon = L.icon.pulse({iconSize:[12,12],color:'white'}); 
				mark1 = L.marker([json_village_group2[j].x, json_village_group2[j].y],{icon: pulsingIcon}).addTo(village_cluster).on('click', onClick_village);
    ...
    map.fitBounds(village_cluster.getBounds());  
}
...
function rightPanel(cult,pays,produit,supplier,farmer,surface) {
	var resurl='playground_listeslies.php?elemid=rightPanel&culture='+cult+'&nbre_pays='+pays+'&nbre_produit='+produit+'&nbre_supplier='+supplier+'&nbre_farmer='+farmer+'&nbre_surface='+surface;
	var xhr = getXhr();
...
function voletgauche(param,valeur) {
	...
	document.getElementById('rightInfos').innerHTML = '<img height="200px" src="img/playground/hma.jpg" style="width:100%;"/><br>'
...
function voletdroit(param,valeur) {
	...
	var resurl='playground_listeslies.php?elemid=voletdroit&param='+param+'&valeur='+valeur;
	var xhr = getXhr();
...
function founisseur(code) {
	var resurl='playground_listeslies.php?elemid=founisseur&code='+code;
	var xhr = getXhr();
		...
		if(xhr.readyState == 4 ){
			...
			if(data[0]!=''){
				...
				while (data[i] != 'end') {
					...
					mark = L.marker([val[0], val[1]],{icon: new SweetIcon({iconUrl: 'playground/images/icon.png',labelText:val[2]})   ,riseOnHover:false}).addTo(labels);
...
function zoomfournisseur(id) {
	...
	var resurl='playground_listeslies.php?elemid=zoomfournisseur&id='+id;
	var xhr = getXhr();
...
function zone(code){
	var resurl='playground_listeslies.php?elemid=zone&code='+code;
	var xhr = getXhr();
...
function histoire(id_histoire){
	...
	var resurl='playground_listeslies.php?elemid=historique&id_histoire='+id_histoire+'&code_pays='+var_pays+'&culture='+culture;
	var xhr = getXhr();
	...
	$.ajax({
		url: "playground/data/story_path/"+id_histoire+".geojson", //or your url
		success: function(data){
			$.getJSON("playground/data/story_path/"+id_histoire+".geojson", function (data) {
				trace.addData(data);
			});
		},
		error: function(data){
...
function bottomBoxes(code_pays,culture) {
	var resurl='playground_listeslies.php?elemid=bottomBoxes&code='+code_pays+'&cult='+culture;
	var xhr = getXhr();
...
var overlayMaps = {
	...
	"PROTECTED AREA": {
       // "&nbsp;&nbsp;<img src='img/playground/parc.png' width='20' height='20'>&nbsp;National Parc": parc_national,
      // "&nbsp;&nbsp;<img src='img/playground/reserve.png' width='20' height='20'>&nbsp;National Reserve": reserve,
      "&nbsp;&nbsp;<img src='img/playground/classified.png' width='20' height='20'>&nbsp;Classified Area": foret_classee
     },
	"OTHERS": {
	   "&nbsp;&nbsp;Time zone": time_zone,
       "&nbsp;&nbsp;Day/Night Zones": t
...
function formatState (state) {
	if(state.text == 'Fr'){
		var $state = $('<span style="z-index:100000000"><img width="20" src="img/playground/fr.jpg" class="img-flag" alt="Français"/> Fr</span>');

	} else

	if(state.text == 'En'){
		var $state = $('<span><img width="20" src="img/playground/en.jpg" class="img-flag" alt="English"/> En</span>');

	} else

	if(state.text == 'De'){
		var $state = $('<span><img width="20" src="img/playground/de.jpg" class="img-flag" alt="Deutch"/> De</span>');
